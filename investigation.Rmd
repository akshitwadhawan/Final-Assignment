...
roc_auc(pred_test, truth = offence_type, `.pred_Violent`)

## ROC curve of only age_group
data_mod_age <- logistic_reg() %>% 
  set_engine("glm")

data_rec_age <- recipe(offence_type ~ age_group, data = data_train)
...
roc_auc(pred_test_age, truth = offence_type, `.pred_Violent`)

## ROC curve of only establishment type
data_mod_est <- logistic_reg() %>% 
  set_engine("glm")

data_rec_est <- recipe(offence_type ~ age_group, data = data_train)
...
roc_auc(pred_test_est, truth = offence_type, `.pred_Violent`)
```
