...
#mutate dataframe into a more useful form
data <- data %>%
  select(`Age group`, Offence) %>%
  mutate(Offence_type = case_when(
    Offence == "Violence" ~ "Violent",
    TRUE ~ "Non-Violent"
  ))

data <- data %>%
  select('Age group', Offence_type)


#change Age group and Offence_type to a factor
data$`Age group` <- as.factor(data$`Age group`)
data$Offence_type <- as.factor(data$Offence_type)
...
data_model <- logistic_reg() %>%
  set_engine("glm") %>%
  fit(Offence_type ~ ., data = train_data, family = 'binomial')
tidy(data_model)

#add predicted probabilities to model
df_pred <- train_data %>%
  bind_cols(
    predict(data_model, train_data, type = "prob")
  ) %>%
  rename(pred_Violent = .pred_Violent, pred_NonViolent = `.pred_Non-Violent`)
```
