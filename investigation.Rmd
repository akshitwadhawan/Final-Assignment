...
#creating a summary statistic to calculate the percentage of all prisoners that fit into each category
...
#creating some summary statistics on this data
...
df_long <- df %>%
  pivot_longer(
    cols = c(`30-jun-2022`, `30-sep-2022`, `31-dec-2022`,
...
ggplot(df_avg, aes(date, value, color = category)) +
...
#summary statistics calculating mean, median and standard deviation based on the sentence length calculated in thousands so easier to interperet
...
roc_auc(pred_test, truth = offence_type, `.pred_Violent`, event_level = "second")

## ROC curve of only age_group
data_mod_age <- logistic_reg() %>% 
  set_engine("glm")

data_rec_age <- recipe(offence_type ~ age_mod, data = data_train) %>%
  step_dummy(all_nominal(), -all_outcomes())

data_wflow_age <- workflow() %>%
  add_recipe(data_rec_age) %>%
  add_model(data_mod_age)

data_fit_age <- fit(data_wflow_age, data_train)

tidy(data_fit_age)

pred_test_age <- predict(
  data_fit_age, data_test, type = "prob"
) %>%
  bind_cols(data_test %>% select(offence_type))
...
roc_auc(pred_test_age, truth = offence_type, `.pred_Violent`, event_level = "second")

## ROC curve of only establishment type
data_mod_est <- logistic_reg() %>% 
  set_engine("glm")

data_rec_est <- recipe(offence_type ~ est_type, data = data_train) %>%
  step_dummy(all_nominal(), -all_outcomes())

data_wflow_est <- workflow() %>%
  add_recipe(data_rec_est) %>%
  add_model(data_mod_est)

data_fit_est <- fit(data_wflow_est, data_train)

tidy(data_fit_est)

pred_test_est <- predict(
  data_fit_est, data_test, type = "prob"
) %>%
  bind_cols(data_test %>% select(offence_type))
...
roc_auc(pred_test_est, truth = offence_type, `.pred_Violent`, event_level = "second")
```
